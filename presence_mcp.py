...
class PresenceMCPServer(MCPServer):
    ...
    def __init__(self, name: str, port: int, presences: List[Presence]):
        ...
        @self.mcp.tool(name="list_tracked_persons", description="Returns a comma-separated list of all names of persons/devices currently being tracked. The entity named 'all' represents the group state (returns 1 if at least one person is present). Use these names as input for 'get_presence_status'.")
